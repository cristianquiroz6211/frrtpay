...
export default function UpdateAffiliate() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.put('https://localhost:8080/general/api/v1/usuarios', dataToSend, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      setFormData({ numeroIdAfiliado: '', correo: '', telefono: '', pin: '', tipoIdentificacionId: '' });
    } catch (error) {
      if (axios.isAxiosError(error)) {
        const errorMessages = error.response?.data?.mensajes || ['Error al actualizar afiliado'];
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      >
        {loading ? 'Actualizando...' : 'Actualizar Afiliado'}
      </button>
    </form>
  );
}